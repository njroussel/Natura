        ...
        switch (dir) {
            case SOUTH: {
                ...
                for (int i = 0; i < m_chunks[0].size(); i++) {
                    m_chunks[0][i] = m_chunk_factory.createChunk(
                            glm::vec2(m_offset.y, i+m_offset.x));
                    m_chunks[0][i]->Init();
                }
                break;
            }
            ...
            case NORTH: {
                m_chunks.push_back(
                        std::deque<Chunk *>(m_chunks[0].size(), NULL));
                for (int i = 0; i < m_chunks[0].size(); i++) {
                    m_chunks[m_chunks.size() -
                             1][i] = m_chunk_factory.createChunk(
                            glm::vec2(m_chunks.size() - 1 + m_offset.y, i+m_offset.x));
            ...
            case WEST: {
                for (int i = 0; i < m_chunks.size(); i++) {
                    m_chunks[i].push_front(
                            m_chunk_factory.createChunk(glm::vec2(i+m_offset.y, m_offset.x)));
                    m_chunks[i][0]->Init();
                }
                break;
            }
            ...
            case EST: {
                for (int i = 0; i < m_chunks.size(); i++) {
                    m_chunks[i].push_back(
                            m_chunk_factory.createChunk(glm::vec2(i+m_offset.y, m_chunks[i].size()-1 + m_offset.x)));
...
private:
    ...
    void _pop_chunks(Direction dir){
        switch (dir){
            case SOUTH: m_chunks.pop_back(); break;
            case NORTH: m_chunks.pop_front(); break;
            case EST:
                for (int i = 0 ; i < m_chunks.size() ; i ++){
